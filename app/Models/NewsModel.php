...
class NewsModel extends AdminModel 
{
    ...
    public function listItems($params = null, $options = null){
        if($options['task'] == 'admin-list-items'){
            $query = $this->select('id','title','img','created','created_by','modified','modified_by','status')            
                            ->where('trash', '=' , 0);
        ...
        if ($options['task'] == 'shop-list-items') {
            $query = $this->select('id','title','img','created','full_text','intro_text')
                            ->where('status', '=', 1)
                            ->where('trash', '=', 0);
            $result = $query->paginate($params['pagination']['totalItemsPerPage']);
        }

        if ($options['task'] == 'shop-list-items-new') {
            $query = $this->select('id','title','img','created','full_text','intro_text')
                            ->where('status', '=', 1)
                            ->where('trash', '=', 0)
    ...
    public function getItem($params = null, $options = null)
    {
        ...
        if ($options['task'] == 'get-item') {
            $result = self::select('id', 'full_text', 'title','intro_text', 'status', 'img','created')->where('id', $params['id'])->first();
        }
